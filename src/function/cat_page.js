...
exports.page = async function (interaction, text, lag) {
    ...
    if (!pages[1]) {
        return interaction.editReply({ content: `page ${page + 1}/${pages.length}` + "```" + `${lag}` + pages[0] + "```" })
    } else {
        ...
        await interaction.editReply({ content: `page ${page + 1}/${pages.length}` + "```" + `${lag}\n` + `${pages[page]}` + "```", components: [new ActionRowBuilder().addComponents([but2, but3, but1])] }).catch(() => { });
        ...
        collector.on("collect", async (button) => {
            if (button.customId === "next") {
                ...
                await interaction.editReply({ content: `page ${page + 1}/${pages.length}` + "```" + `${lag}\n` + `${pages[page]}` + "```", components: [new ActionRowBuilder().addComponents([but2, but3, but1])] })
        ...
        collector.on("end", async () => { await interaction.editReply({ content: `page ${page + 1}/${pages.length}` + "```" + `${lag}\n` + `${pages[page]}` + "```", components: [new ActionRowBuilder().addComponents([dedbut2, dedbut3, dedbut1])] }); })
    }
}
