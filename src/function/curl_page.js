...
exports.page = async function (interaction, text) {
    let pages = []
    let page = 0
    ...
    if (!pages[1]) {
        return interaction.editReply({ content: `page ${page + 1}/${pages.length}` + "```" + pages[0] + "```" })
    } else {
        ...
        await interaction.editReply({ content: `page ${page + 1}/${pages.length}` + "```" + `${pages[page]}` + "```", components: [new ActionRowBuilder().addComponents([but2, but3, but1])] }).catch(() => { });
        ...
        collector.on("collect", async (button) => {
            if (button.customId === "next") {
                ...
                await interaction.editReply({ content: `page ${page + 1}/${pages.length}` + "```" + `${pages[page]}` + "```", components: [new ActionRowBuilder().addComponents([but2, but3, but1])] })
        ...
        collector.on("end", async () => { await interaction.editReply({ content: `page ${page + 1}/${pages.length}` + "```" + `${pages[page]}` + "```", components: [new ActionRowBuilder().addComponents([dedbut2, dedbut3, dedbut1])] }); })
    }
}
